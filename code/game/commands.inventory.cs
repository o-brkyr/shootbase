...
public partial class MyGame
{
    ...
    public static void GiveWeapon(string weaponPrefabName)
    {
        var callingClient = ConsoleSystem.Caller;

        var owner = ConsoleSystem.Caller.Pawn as Shootbase.Player;

        var fullResourcePath = $"prefabs/weapons/{weaponPrefabName.ToLower().Trim()}.prefab";
        ...
        WeaponBase weapon;
        if (!PrefabLibrary.TrySpawn<WeaponBase>(fullResourcePath, out weapon))
        {
            Log.Info("No makey weapon");
            return;
        }

        if (owner.Inventory != null)
        {
            int index = owner.Inventory.AddWeapon(weapon);
            owner.Inventory.SwitchToSlot(index);
        }

        //WeaponBase weapon;
        //if ( !PrefabLibrary.TrySpawn<WeaponBase>( weaponPrefabName, out weapon ) )
        //{
        //	Log.Warning( "Couldn't spawn gun prefab?" );
        //	return;
        //}
    ...
    //	{
    //		var key = kp.Key;
    //		Shootbase.AmmoStorageData item = kp.Value;
